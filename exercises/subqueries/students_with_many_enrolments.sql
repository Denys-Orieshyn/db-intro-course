...
SELECT
    ...
    sc.full_name,
    sc.course_number,
    (SELECT ROUND(AVG(course_number), 2) FROM student_courses) AS avg_number
FROM student_courses sc
WHERE sc.course_number > (SELECT AVG(course_number) FROM student_courses)
ORDER BY sc.course_number DESC, sc.full_name ASC